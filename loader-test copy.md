...
Create `index.html` with the content below. It loads the remote loader at `https://proffyn.click/evi-chatbot-loader.js`, which in turn loads `https://proffyn.click/evi-chatbot.js` and calls `window.EviChatBot.init()`.
...
<html>
  ...
  <body>
    ...
    <main>
      <p>This page loads the chatbot via a remote loader script hosted at <code>proffyn.click</code>.</p>
    ...
    <script src="https://proffyn.click/evi-chatbot-loader.js" async></script>
  </body>
  </html>
```
...
- Open DevTools → Network tab → confirm `evi-chatbot-loader.js` and `evi-chatbot.js` load from `https://proffyn.click/`.
...
<script>
  document.getElementById('show-chat').addEventListener('click', function () {
    ...
    s.src = 'https://proffyn.click/evi-chatbot-loader.js';
    s.async = true;
...
- CSP blocking remote scripts: if your host enforces a strict Content Security Policy, allow `https://proffyn.click` for `script-src`, or host a local copy of `evi-chatbot-loader.js`.
- Ad/script blockers: try an incognito window or different browser/profile.
- CORS: already configured for `https://proffyn.click`. If you move hosts, update CORS on the n8n endpoint accordingly.
